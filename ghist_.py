...
def data_for_header(serial,serial_id = None):
    con = db.get_connection()
    cur = con.cursor()
    cur.execute(" select * from usadd_web.ghist_list (?,?) ",[serial.strip(),serial_id])
    rows = cur.fetchall()
...
def index():
    ...
    if request.method == "POST":
        ...
        # con.close()
        global data
        data = data_for_header(search_str)
...
def datails(search_str):
    ...
    data = data_for_header('0',search_str)
    print(data[0])
    con.close()
    return render_template('ghist_det.html',title=title,rows = data_movies,row=data[0] )
